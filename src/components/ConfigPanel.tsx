...
export function ConfigPanel() {
  ...
  // Live per-node metrics need monitoring (basic+); the bottleneck reason is a
  // trace-level detail, so it needs full observability.
  const showLiveStats = runPhase !== "build" && loaded && obs !== "none";
  ...
  return (
    <aside className="flex w-80 shrink-0 flex-col border-l border-border bg-card/30">
      ...
      {showLiveStats && obs === "full" && r?.bottleneck && (
